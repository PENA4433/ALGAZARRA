...
// Apenas professores (nivel 1) podem acessar esta página
if ($_SESSION['nivel'] != 1) {
    $_SESSION['err'] = "Acesso negado. Apenas professores podem inscrever alunos.";
...
if (isset($_POST['id_aluno']) && isset($_POST['id_atividade'])) {
    ...
    if (mysqli_num_rows($res_check) > 0) {
        ...
        header("Location: inscrever_aluno_professor.php");
        exit;
    }
...
$stmt_encarregados = mysqli_prepare($conn, "SELECT id, nome FROM enc_educacao ORDER BY nome");
mysqli_stmt_execute($stmt_encarregados);
$result_encarregados = mysqli_stmt_get_result($stmt_encarregados);
mysqli_stmt_close($stmt_encarregados);
...
// Variável para armazenar alunos do encarregado selecionado
$alunos_encarregado = array();
if (isset($_POST['id_encarregado'])) {
    $id_encarregado = intval($_POST['id_encarregado']);
    $stmt_alunos = mysqli_prepare($conn, "SELECT id, nome, data_nascimento FROM aluno WHERE enc_educacao = ? ORDER BY nome");
    mysqli_stmt_bind_param($stmt_alunos, "i", $id_encarregado);
    ...
    while ($aluno = mysqli_fetch_assoc($result_alunos)) {
        $alunos_encarregado[] = $aluno;
    }
}
?>

    <meta charset="UTF-8">
    <title>Inscrever Aluno - Professor</title>
...
<form method="post">
    ...
    <div class="row">
        <div class="col-md-6 mt-3">
            <label for="id_encarregado" class="form-label">Selecione o encarregado de educação:</label>
            <select name="id_encarregado" id="id_encarregado" class="form-control" required onchange="this.form.submit();">
                <option value="">-- Escolha um encarregado --</option>
                <?php 
                // Reposicionar para o início
                mysqli_data_seek($result_encarregados, 0);
                while ($encarregado = mysqli_fetch_assoc($result_encarregados)) { 
                    $selected = (isset($_POST['id_encarregado']) && $_POST['id_encarregado'] == $encarregado['id']) ? 'selected' : '';
                ?>
                    <option value="<?php echo htmlspecialchars($encarregado['id']); ?>" <?php echo $selected; ?>>
                        <?php echo htmlspecialchars($encarregado['nome']); ?>
                    </option>
                <?php } ?>
            </select>
        </div>
    ...
    <?php if (!empty($alunos_encarregado)) { ?>
        <div class="row">
            <div class="col-md-6 mt-3">
                ...
                <select name="id_aluno" id="id_aluno" class="form-control" required>
                    ...
                    <?php foreach ($alunos_encarregado as $aluno) { ?>
            ...
            <div class="col-md-6 mt-3">
                ...
                <select name="id_atividade" id="id_atividade" class="form-control" required>
                    ...
                    <?php 
                    // Reposicionar para o início
                    mysqli_data_seek($result_atividades, 0);
