...
if ($nivel == 2) {
    ...
    $stmt_aluno = mysqli_prepare($conn, "SELECT id, nome FROM aluno WHERE id = ? AND enc_educacao = ?");
    ...
    $stmt_aluno = mysqli_prepare($conn, "SELECT id, nome FROM aluno WHERE id = ?");
...
if (isset($_POST['id_atividade'])) {
    ...
    $stmt_atividade = mysqli_prepare($conn, "SELECT id, lotacao_max FROM atividade WHERE id = ?");
    ...
    /* verificar duplicado */
    $stmt_check = mysqli_prepare($conn, "SELECT aluno FROM inscricao WHERE aluno = ? AND atividade = ? LIMIT 1");
    ...
    /* verificar lotação */
    $stmt_lotacao = mysqli_prepare($conn, "SELECT COUNT(DISTINCT aluno) AS total_inscritos FROM inscricao WHERE atividade = ?");
...
<h5 class="text-center text-muted">Aluno: <?php echo htmlspecialchars($aluno_atual['nome']); ?></h5>
