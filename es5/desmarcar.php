...
$stmt = mysqli_prepare($conn, "UPDATE inscricao SET esta_presente = 0 WHERE aluno = ? AND atividade = ? AND dia = ?");
mysqli_stmt_bind_param($stmt, "iis", $id_aluno, $id_atividade, $dia);
$retVal = mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);

if (!$retVal) {
    $_SESSION['erro'] = 'Erro ao desmarcar presença.';
    header('Location: erro.php');
    exit;
}

if (mysqli_affected_rows($conn) > 0) {
    $_SESSION['info'] = 'Desmarcou a presença no dia ' . date('d/m/Y', strtotime($dia)) . '.';
} else {
    $_SESSION['info'] = 'Presença já estava desmarcada ou registo não encontrado.';
}
...
<xai:function_call name="edit_file">
<parameter name="path">/Applications/XAMPP/xamppfiles/htdocs/pwbd/otl/TODO.md
